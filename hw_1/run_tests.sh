...
# функция для сравнения вывода cli скрипта и Linux-команды
compare_with_linux() {
...
run_test 'touch artifacts/empty.txt; python3 cli.py nl artifacts/empty.txt'
compare_with_linux 'touch artifacts/empty.txt; nl -b a artifacts/empty.txt'

# tail
run_test 'seq 1 20 | python3 cli.py tail'
compare_with_linux 'seq 1 20 | tail -n 17'

run_test 'seq 1 15 > artifacts/test.txt; python3 cli.py tail artifacts/test.txt'
compare_with_linux 'seq 1 15 > artifacts/test.txt; tail -n 10 artifacts/test.txt'

run_test 'seq 1 5 > artifacts/test1.txt; seq 6 10 > artifacts/test2.txt; python3 cli.py tail artifacts/test1.txt artifacts/test2.txt'
...
run_test 'touch artifacts/empty.txt; python3 cli.py tail artifacts/empty.txt'
...
run_test 'touch artifacts/empty.txt; python3 cli.py wc artifacts/empty.txt'
...
run_test 'touch artifacts/empty1.txt artifacts/empty2.txt; python3 cli.py nl artifacts/empty1.txt; python3 cli.py tail artifacts/empty1.txt artifacts/empty2.txt; python3 cli.py wc artifacts/empty1.txt artifacts/empty2.txt'
